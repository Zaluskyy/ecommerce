...
const AccountSettings: React.FC<AccountSettingsProps> = () => {
  ...
  const dataArr: IDataArr[] = [
    {
      title: "Your data",
      data: displayName,
      data2: `${displayTelephone ? `Tel. ${displayTelephone}` : ""}`,
    },
    ...
    return (
      <div key={item.title} className={style.tableContainer}>
        ...
        <div className={style.container}>
          ...
          {index !== 1 && (
            <span
              onClick={() => setEdit(index == 2 ? 1 : index)}
  ...
  const handleDeleteAccount = async () => {
    ...
    if (user) {
      ...
      const isGoogleAuth = providerData.some(
    ...
    <div className={style.AccountSettings}>
      ...
      <div className={style.deleteAccount}>
        ...
        <span className={style.text}>
          ...
          Please make sure you really want to do this – we won&apos;t be able to
          restore your account
        </span>
        <ButtonAnimation onClick={handleDeleteAccount}>
          Delete account
        </ButtonAnimation>
      </div>
