...
const EditDataPopUp: FC<EditDataPopUpProps> = ({
  setEditData,
  title,
  initialValues,
  validationSchema,
  ...
  const handleSave = async (values: any) => {
    ...
    if (title == "Your data") {
      ...
      const { fullName, street, apartmentNumber, zipCode, city, telephone } =
        values;
      if (user) {
        const userDocRef = doc(db, "account", user.uid);
        const userDocSnapshot = await getDoc(userDocRef);
        if (userDocSnapshot.exists()) {
          ...
          await setDoc(
            userDocRef,
            { street, apartmentNumber, zipCode, city, telephone },
            { merge: true }
          );
          toast.success("Updated your data");
        } else {
                  ...
                  <div key={index}>
                    ...
                    <div className={style.errorContainer}>
                      {errors[item.name] && touched[item.name] && (
                        <span className={style.error}>{errors[item.name]}</span>
                      )}
                    </div>
                  </div>
                );
